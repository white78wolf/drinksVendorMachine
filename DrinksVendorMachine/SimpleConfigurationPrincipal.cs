...
namespace DrinksVendorMachine
{
    ...
    public class SimpleConfigurationPrincipal : IPrincipal
    {
        ...
        public SimpleConfigurationPrincipal(IIdentity identity)
        {
            if (identity == null)
            {
                //throw new ArgumentNullException(nameof(identity)); // ругается на nameof
            }

            this.Identity = identity;

            string roleSetting = String.Empty;
            if(identity != null)
                roleSetting = string.Format("roles:{0}", identity.Name); //
            var roles = ConfigurationManager.AppSettings[roleSetting];

            if (!string.IsNullOrEmpty(roles))
            {
                foreach (var role in roles.Split('|')) // после roles здесь был "?", ругалось на него
                {
                    _roles.Add(role);
                }
            }
